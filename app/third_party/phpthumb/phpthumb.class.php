...
//                                                         ///
//////////////////////////////////////////////////////////////
function __escapeshellarg($input)
{
	...
	function file_exists_ignoreopenbasedir($filename, $cached=true) {
		...
		if (!$cached || !isset($file_exists_cache[$filename])) {
			...
			if (empty($open_basedirs) || in_array(dirname($filename), $open_basedirs)) {
				...
				$ls_filename = trim(phpthumb_functions::SafeExec('dir '.__escapeshellarg($filename)));
				$file_exists_cache[$filename] = !preg_match('#File Not Found#i', $ls_filename);
			} else {
				$ls_filename = trim(phpthumb_functions::SafeExec('ls '.__escapeshellarg($filename)));
	...
	function ImageMagickCommandlineBase() {
		...
		if (is_null($commandline)) {
			...
			if ($this->file_exists_ignoreopenbasedir($this->config_imagemagick_path)) {
				...
				if ($this->iswindows) {
					$commandline = substr($this->config_imagemagick_path, 0, 2).' && cd '.__escapeshellarg(str_replace('/', DIRECTORY_SEPARATOR, substr(dirname($this->config_imagemagick_path), 2))).' && '.__escapeshellarg(basename($this->config_imagemagick_path));
				} else {
					$commandline = __escapeshellarg($this->config_imagemagick_path);
				}
		...
		if ($commandline) {
			if ($IMtempfilename = $this->phpThumb_tempnam()) {
				...
				if ($this->ImageMagickSwitchAvailable('thumbnail') && $this->config_imagemagick_use_thumbnail) {
					...
					$commandline_test = $this->ImageMagickCommandlineBase().' logo: -resize 1x '.__escapeshellarg($IMtempfilename).' 2>&1';
				...
				if (!is_null($this->dpi) && $this->ImageMagickSwitchAvailable('density')) {
					...
					$commandline .= ' -density '.__escapeshellarg($this->dpi);
				}
				ob_start();
				...
				if (is_array($getimagesize)) {
					...
					if (!preg_match('#('.implode('|', $this->AlphaCapableFormats).')#i', $outputFormat)) {
						...
						$commandline .= ' -background '.__escapeshellarg('#'.($this->bg ? $this->bg : 'FFFFFF'));
					...
					if ($this->source_width || $this->source_height) {
						if ($this->zc) {
							...
							$commandline .= ' -'.$IMresizeParameter.' '.__escapeshellarg(($IMuseExplicitImageOutputDimensions ? $thumbnailH : '').'x'.$thumbnailH);
							...
							if (($wAll > 0) && ($hAll > 0)) {
								$commandline .= ' -crop '.__escapeshellarg($wAll.'x'.$hAll.'+0+0');
							} else {
								$commandline .= ' -crop '.__escapeshellarg($side.'x'.$side.'+0+0');
							}
...
// '/usr/bin/convert' -density 150 -thumbnail 100x100 -contrast-stretch '0.1%' '/var/www/vhosts/trainspotted.com/httpdocs/content/CNR/47/CNR-4728-LD-L-20110723-898.jpg[0]' png:'/var/www/vhosts/trainspotted.com/httpdocs/phpThumb/_cache/pThumbIIUlvj'
							$commandline .= ' -crop '.__escapeshellarg($crop_param);
...
// newer versions of ImageMagick require -repage <geometry>
								...
								$commandline .= ' -'.$IMresizeParameter.' '.__escapeshellarg($this->w.'x'.$this->h);
							}
							...
							if ($this->iar && (intval($this->w) > 0) && (intval($this->h) > 0)) {
								...
								$commandline .= ' -'.$IMresizeParameter.' '.__escapeshellarg($nw.'x'.$nh.'!');
							} else {
					...
					if ($this->w || $this->h) {
						...
						if ($IMuseExplicitImageOutputDimensions) {
							...
							$commandline .= ' -'.$IMresizeParameter.' '.__escapeshellarg(($this->w ? $this->w : '9999').'x'.($this->h ? $this->h : '9999').$exactDimensionsBang);
						} else {
							$commandline .= ' -'.$IMresizeParameter.' '.__escapeshellarg($this->w.'x'.$this->h.$exactDimensionsBang);
						}
					}
				...
				if ($this->ra) {
					...
					if ($this->ImageMagickSwitchAvailable('rotate')) {
						if (!preg_match('#('.implode('|', $this->AlphaCapableFormats).')#i', $outputFormat) || phpthumb_functions::version_compare_replacement($this->ImageMagickVersion(), '6.3.7', '>=')) {
							...
							$commandline .= ' -rotate '.__escapeshellarg($this->ra);
							if (($this->ra % 90) != 0) {
								if (preg_match('#('.implode('|', $this->AlphaCapableFormats).')#i', $outputFormat)) {
					...
					switch ($command) {
						case 'brit':
							if ($this->ImageMagickSwitchAvailable('modulate')) {
								$commandline .= ' -modulate '.__escapeshellarg((100 + intval($parameter)).',100,100');
						...
						case 'ds':
							if ($this->ImageMagickSwitchAvailable(array('colorspace', 'modulate'))) {
								if ($parameter == 100) {
									...
									$commandline .= ' -modulate '.__escapeshellarg('100,'.(100 - intval($parameter)).',100');
								}
								$successfullyProcessedFilters[] = $filterkey;
							}
							break;
						...
						case 'clr':
							if ($this->ImageMagickSwitchAvailable(array('fill', 'colorize'))) {
								...
								$commandline .= ' -fill '.__escapeshellarg('#'.preg_replace('#[^0-9A-F]#i', '', $color));
								$commandline .= ' -colorize '.__escapeshellarg(min(max(intval($amount), 0), 100));
							}
							break;
						...
						case 'sep':
							if ($this->ImageMagickSwitchAvailable('sepia-tone')) {
								...
								if (!$color) {
									$commandline .= ' -sepia-tone '.__escapeshellarg(min(max(intval($amount), 0), 100).'%');
									$successfullyProcessedFilters[] = $filterkey;
								}
							}
							break;
						...
						case 'gam':
							...
							if (number_format($amount, 3) != '1.000') {
								if ($this->ImageMagickSwitchAvailable('gamma')) {
									$commandline .= ' -gamma '.__escapeshellarg($amount);
									$successfullyProcessedFilters[] = $filterkey;
								}
							}
							break;
						...
						case 'th':
							...
							if ($this->ImageMagickSwitchAvailable(array('threshold', 'dither', 'monochrome'))) {
								$commandline .= ' -threshold '.__escapeshellarg(round(min(max(intval($amount), 0), 255) / 2.55).'%');
								$commandline .= ' -dither';
						...
						case 'rcd':
							if ($this->ImageMagickSwitchAvailable(array('colors', 'dither'))) {
								...
								$commandline .= ' -colors '.__escapeshellarg(max($colors, 8)); // ImageMagick will otherwise fail with "cannot quantize to fewer than 8 colors"
						...
						case 'edge':
							if ($this->ImageMagickSwitchAvailable('edge')) {
								...
								$commandline .= ' -edge '.__escapeshellarg(!empty($parameter) ? intval($parameter) : 1);
								$successfullyProcessedFilters[] = $filterkey;
							}
							break;
						...
						case 'emb':
							if ($this->ImageMagickSwitchAvailable(array('emboss', 'negate'))) {
								$parameter = (!empty($parameter) ? $parameter : 2);
								$commandline .= ' -emboss '.__escapeshellarg(intval($parameter));
								if ($parameter < 2) {
							...
							switch ($method) {
								...
								case 2: // ImageMagick "contrast-stretch"
									if ($this->ImageMagickSwitchAvailable('contrast-stretch')) {
										if ($band != '*') {
											$commandline .= ' -channel '.__escapeshellarg(strtoupper($band));
										}
										$threshold = preg_replace('#[^0-9\\.]#', '', $threshold); // should be unneccesary, but just to be double-sure
										//$commandline .= ' -contrast-stretch '.__escapeshellarg($threshold.'%');
						...
						case 'wb':
							if ($this->ImageMagickSwitchAvailable(array('channel', 'contrast-stretch'))) {
								...
								$threshold = preg_replace('#[^0-9\\.]#', '', $threshold); // should be unneccesary, but just to be double-sure
								//$commandline .= ' -channel R -contrast-stretch '.__escapeshellarg($threshold.'%'); // doesn't work on Windows because most versions of PHP do not properly
								//$commandline .= ' -channel G -contrast-stretch '.__escapeshellarg($threshold.'%'); // escape special characters (such as %) and just replace them with spaces
								//$commandline .= ' -channel B -contrast-stretch '.__escapeshellarg($threshold.'%'); // https://bugs.php.net/bug.php?id=43261
						...
						case 'blur':
							if ($this->ImageMagickSwitchAvailable('blur')) {
								...
								$commandline .= ' -blur '.__escapeshellarg($radius);
								$successfullyProcessedFilters[] = $filterkey;
							}
							break;
						...
						case 'gblr':
							...
							if ($this->ImageMagickSwitchAvailable('gaussian-blur')) {
								$commandline .= ' -gaussian-blur '.__escapeshellarg($radius);
								$successfullyProcessedFilters[] = $filterkey;
							} elseif ($this->ImageMagickSwitchAvailable('gaussian')) {
								$commandline .= ' -gaussian '.__escapeshellarg($radius);
								$successfullyProcessedFilters[] = $filterkey;
							}
							break;
						...
						case 'usm':
							if ($this->ImageMagickSwitchAvailable('unsharp')) {
								...
								$commandline .= ' -unsharp '.__escapeshellarg(number_format(($radius * 2) - 1, 2, '.', '').'x1+'.number_format($amount / 100, 2, '.', '').'+'.number_format($threshold / 100, 2, '.', ''));
								$successfullyProcessedFilters[] = $filterkey;
							}
							break;
							...
							if ($this->ImageMagickSwitchAvailable(array('border', 'bordercolor', 'thumbnail', 'crop'))) {
								if (!$this->zc) {
									...
									if ($width && !$rX && !$rY) {
										...
										$commandline .= ' -border '.__escapeshellarg(intval($width));
										$commandline .= ' -bordercolor '.__escapeshellarg('#'.$color);

										if (preg_match('# \\-crop "([0-9]+)x([0-9]+)\\+0\\+0" #', $commandline, $matches)) {
											$commandline = str_replace(' -crop "'.$matches[1].'x'.$matches[2].'+0+0" ', ' -crop '.__escapeshellarg(($matches[1] - (2 * $width)).'x'.($matches[2] - (2 * $width)).'+0+0').' ', $commandline);
										} elseif (preg_match('# \\-'.$IMresizeParameter.' "([0-9]+)x([0-9]+)" #', $commandline, $matches)) {
											$commandline = str_replace(' -'.$IMresizeParameter.' "'.$matches[1].'x'.$matches[2].'" ', ' -'.$IMresizeParameter.' '.__escapeshellarg(($matches[1] - (2 * $width)).'x'.($matches[2] - (2 * $width))).' ', $commandline);
										}
										$successfullyProcessedFilters[] = $filterkey;
									}
								}
				...
				if (preg_match('#jpe?g#i', $outputFormat) && $this->q) {
					if ($this->ImageMagickSwitchAvailable(array('quality', 'interlace'))) {
						$commandline .= ' -quality '.__escapeshellarg($this->thumbnailQuality);
				...
				$commandline .= ' '.__escapeshellarg(preg_replace('#[/\\\\]#', DIRECTORY_SEPARATOR, $this->sourceFilename).(($outputFormat == 'gif') ? '' : '['.intval($this->sfn).']')); // [0] means first frame of (GIF) animation, can be ignored
				$commandline .= ' '.$outputFormat.':'.__escapeshellarg($IMtempfilename);
				if (!$this->iswindows) {
					$commandline .= ' 2>&1';
				}
